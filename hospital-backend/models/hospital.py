...
Defines the SQLAlchemy model for inpatient Hospital institutions.
"""
...
class Hospital(db.Model):
    ...
    Hospitals provide acute care, intensive care units, surgeries,
    and 24/7 emergency medical services with inpatient bed allocations.
    """
    ...
    __table_args__ = (
        ...
        db.CheckConstraint("latitude >= -90.0 AND latitude <= 90.0", name="check_hospital_lat_range"),
        db.CheckConstraint("longitude >= -180.0 AND longitude <= 180.0", name="check_hospital_lng_range"),
    )
    ...
    def validate_beds(self, key, value):
        if value is None:
            raise ValueError("Hospital 'beds' count cannot be null.")
        try:
            val_int = int(value)
        except (ValueError, TypeError):
            raise ValueError(f"Hospital 'beds' must be an integer, got: {value}")
        if val_int < 0:
            raise ValueError(f"Hospital 'beds' cannot be negative. Got: {val_int}")
        return val_int

    @validates("latitude")
    def validate_latitude(self, key, value):
        if value is None:
            raise ValueError("Hospital 'latitude' is required.")
        try:
    ...
    def validate_longitude(self, key, value):
        if value is None:
            raise ValueError("Hospital 'longitude' is required.")
        try:
    ...
    def to_geojson_feature(self):
        """Serialize hospital to an RFC 7946 GeoJSON Feature."""
        return {
            "type": "Feature",
            "id": f"hospital-{self.id}",
            "geometry": {
                "type": "Point",
