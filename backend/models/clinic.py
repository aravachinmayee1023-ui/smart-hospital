"""
Clinic Model Module
Defines the SQLAlchemy model for Outpatient & Ambulatory Clinics in the medical network.
"""
...
class Clinic(db.Model):
    """
    SQLAlchemy model representing an Ambulatory Clinic or Urgent Care Center.
    
    Clinics typically provide outpatient care, specialized diagnostics, or minor procedures,
    often with lower observation bed counts and non-emergency operating hours.
    """
    ...
    # 4. Observation / Day Bed Capacity
    ...
    def validate_name(self, key, value):
        """Validate that the clinic name is provided and non-empty."""
    ...
    def validate_type(self, key, value):
        """Validate that the clinic type is provided and non-empty."""
        if not value or not str(value).strip():
    ...
    def to_dict(self):
        """Serialize clinic record to a JSON-ready dictionary."""
        return {
            "id": self.id,
            "name": self.name,
            "type": self.type,
